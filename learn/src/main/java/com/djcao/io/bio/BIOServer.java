...
public class BIOServer {
                ...
                executorService.scheduleAtFixedRate(() -> {
                    try {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            if (socket != null) {
                                connectedSocket.put(getRemoteInstanceId(
                                    (InetSocketAddress) socket.getRemoteSocketAddress()), socket);
                                iterator.remove();
                            }
                        }
    ...
    public List<MutablePair<Event, Socket>> getEmmitEventSocket() throws IOException {
        List<MutablePair<Event, Socket>> rlt = new ArrayList<>();
        Iterator<Socket> iterator = connectedSocket.values().iterator();
        while (iterator.hasNext()) {
            Socket next = iterator.next();
            MutablePair<Event, Socket> event = findEvent(next);
            if (event != null) {
                rlt.add(event);
            }
        }
    ...
    public void sout(List<MutablePair<Event, Socket>> eventSocket) {
        System.out.println("开始一轮轮询,size:" + eventSocket.size());
        Iterator<MutablePair<Event, Socket>> iterator = eventSocket.iterator();
        while (iterator.hasNext()) {
            MutablePair<Event, Socket> next = iterator.next();
            switch (next.getKey()) {
                case CONNECTED:
    ...
    public void processUnConnect(Socket socket) {
        ...
        System.out
            .println(socketAddress.getHostString() + ":" + socketAddress.getPort() + "断开连接成功");
    }

    public MutablePair<Event, Socket> findEvent(Socket socket) throws IOException {
        if (socket.isClosed()) {
            return new MutablePair<>(Event.UNCONNECTED, socket);
        } else if (socket.getInputStream().available() > 0) {
            return new MutablePair<>(Event.READABLE, socket);
        } else {
            //pass
        }
        return null;
